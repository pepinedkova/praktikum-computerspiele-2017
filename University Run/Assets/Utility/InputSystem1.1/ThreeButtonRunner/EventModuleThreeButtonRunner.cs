...
{
    public class EventModuleThreeButtonRunner : AEventModule
    {
        private Image[] buttons;

        public EventModuleThreeButtonRunner(IEventHandlerThreeButtonRunner reciever, Image[] buttons)
        {
        ...
        public override bool SearchEvent(List<TouchData> touchDatas, float tDelta)
        {
            ...
            {
                vecMove.Normalize();
                (reciever as IEventHandlerThreeButtonRunner).OnButtonPress(vecMove, isJumping);
                return true;
            }
